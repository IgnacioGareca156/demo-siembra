...
import EditarBiblioteca from './EditarBiblioteca'
import EliminarBoblioteca from './EliminarBiblioteca'
import NuevaBiblioteca from './NuevaBiblioteca'



const TablaBibliotecas = () => {
    ...
    useEffect(() => {
        ...
        if(isAdmin){
            get('Biblioteca', 'GET_BIBLIOTECAS', {
                limit:-1,
            })
        }else{
            get('Biblioteca', 'GET_BIBLIOTECAS', {
    ...
    const columns = [
        { id: 'biblio_titulo', label: 'Título', minWidth: 170 },
        { id: 'biblio_descripcion', label: 'Descripción', minWidth: 170 },
        {
            id: 'biblio_materia',
            label: 'Materia',
            minWidth: 170,
    ...
    useEffect(() => {
        ...
        setTimeout(() => {
            setRows(
                state.bibliotecas?.map((biblio) => {
                    // Buscar el nombre del aula y materia correspondiente
                    const aula = state.aulas.find(aula => aula.aula_id === biblio.biblio_aula)
                    const materia = state.materias.find(materia => materia.materia_id === biblio.biblio_materia)
    
                     // Obtener día, mes y año
                     const fecha = new Date(biblio.date_created)
                    ...
                    return {
                        ...
                        biblio_titulo: biblio.biblio_titulo,
                        biblio_descripcion: biblio.biblio_descripcion,
                        biblio_aula_id: biblio.biblio_aula?.aula_id,
                        biblio_materia_id: biblio.biblio_materia?.materia_id,
                        biblio_file: biblio.biblio_file,
                        biblio_aula: aula?.aula_nombre || '',
                        biblio_materia: materia?.materia_nombre || '',
    ...
    return isDesktop ? (
        ...
        <Paper sx={{ width: '100%', overflow: 'hidden' }}>
                ...
                <Table stickyHeader aria-label="sticky table">
                    ...
                    <TableBody>
                        {sortedRows()
                            ...
                            ?.map((row) => (
                                <TableRow hover role="checkbox" tabIndex={-1} key={row.biblio_id} >
                                    {columns?.map((column) => (
                                            ...
                                            {(() => {
                                                ...
                                                if (column.id === 'acciones') {
                                                    if (profesor?.profesor_admin) {
                                                        ...
                                                        return (
                                                            <>
                                                                <EditarBiblioteca biblio_id={row.biblio_id} aulas={state.aulas} materias={state.materias} />
                                                                <EliminarBoblioteca biblio_id={row.biblio_id} biblio_file={row.biblio_file} />
                                                                <IconButton component={'a'} color='' aria-label='Ver Archivo' size='large' target="_blank" href={`https://xer.pascalito.com.ar/assets/${row.biblio_file}`}>
            ...
            <TablePagination
                ...
                component="div"
                count={state.bibliotecas?.length}
                                                    ...
                                                    if (profesor?.profesor_admin) {
                                                        return (
                                                            ...
                                                            >
                                                                <EditarBiblioteca biblio_id={row.biblio_id} aulas={state.aulas} materias={state.materias} />
                                                                <EliminarBoblioteca biblio_id={row.biblio_id} biblio_file={row.biblio_file} />
                                                                <IconButton component={'a'} color='' aria-label='Ver Cartilla' size='large' target="_blank" href={`https://xer.pascalito.com.ar/assets/${row.biblio_file}`}>
                                                                    <RemoveRedEyeSharpIcon />
                                                                </IconButton>
                                                            </TableCell>
                                                        )
...
export default TablaBibliotecas
